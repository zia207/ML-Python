...
Where:
- $\alpha$ = learning rate
- $r + \gamma \cdot \max_{a'} Q(s', a')$ = TD target (Temporal Difference)
- $r + \gamma \cdot \max_{a'} Q(s', a') - Q(s, a)$ = TD error

**Exploration vs Exploitation**: $\varepsilon$-greedy policy:
- With probability $\varepsilon$: explore (random action)
- With probability $1-\varepsilon$: exploit (best known action)
...
**→ Next:** [Chapter 15: Model Evaluation & Best Practices](chapter-15-lstm.qmd)
