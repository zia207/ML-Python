...
subtitle: "Math refresher and the Python toolkit we use throughout"
date: last-modified
categories: [beginner, setup, math, python]
toc: true
toc-depth: 3
...
This tutorial assumes: - Basic Python syntax (variables, loops, functions, lists) - High school algebra (equations, functions, graphs) - No prior machine learning knowledge required
...
In ML, data is structured as: - **Feature matrix X**: shape `(n_samples, n_features)` — each row is one observation - **Target vector y**: shape `(n_samples,)` — what we want to predict
