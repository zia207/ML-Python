...
- As z → +∞: σ(z) → 1
- As z → -∞: σ(z) → 0
- Derivative: σ'(z) = σ(z)(1 − σ(z))  ← very useful for backprop!
...
### Decision Rule
```
Predict class 1 if ŷ ≥ 0.5  (i.e., z ≥ 0)
Predict class 0 if ŷ < 0.5  (i.e., z < 0)
```
...
```
J(w) = -(1/n) Σᵢ [ yᵢ log(ŷᵢ) + (1-yᵢ) log(1-ŷᵢ) ]
```

**Intuition**:
- When y=1: loss = -log(ŷ) → penalizes predictions close to 0 heavily
- When y=0: loss = -log(1-ŷ) → penalizes predictions close to 1 heavily
...
| Sigmoid | `σ(z) = 1/(1+e⁻ᶻ)` |
| Model | `ŷ = σ(wᵀx)` |
| Loss | `J = -(1/n) Σ [y log ŷ + (1-y) log(1-ŷ)]` |
| Gradient | `∂J/∂w = (1/n) Xᵀ(ŷ - y)` |

---

## 📝 Exercises

1. Plot the sigmoid function for z ∈ [−10, 10].
...
**← Previous:** [Chapter 02: Linear Regression](chapter-02-linear-regression.md)  
**→ Next:** [Chapter 04: K-Nearest Neighbors](chapter-04-knn.md)
